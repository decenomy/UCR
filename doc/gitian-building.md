...
*Setup instructions for a gitian build of UltraClear using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the UltraClear
...
- [Building UltraClear](#building-ucr)
...
Clone the git repositories for ucr and gitian and then checkout the ucr version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ucr-crypto/clearcore-project.git
cd ucr
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ucr repository
...
To build UltraClear (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ucr repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ucr/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/ucr-crypto/ucr
...
./bin/gbuild --commit ucr=${COMMIT} --url ucr=${URL} ../ucr/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ucr=${COMMIT} --url ucr=${URL} ../ucr/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ucr=${COMMIT} --url ucr=${URL} ../ucr/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ucr-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ucr-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ucr-build.assert
```
...
[ucr/gitian.sigs](https://github.com/ucr-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
