...
#include "ui_zucrcontroldialog.h"
...
std::set<std::string> ZUcrControlDialog::setSelectedMints;
std::set<CMintMeta> ZUcrControlDialog::setMints;

bool CZUcrControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZUcrControlDialog::COLUMN_DENOMINATION || column == ZUcrControlDialog::COLUMN_VERSION || column == ZUcrControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZUcrControlDialog::ZUcrControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZUcrControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZUcrControlDialog::~ZUcrControlDialog()
{
    delete ui;
}

void ZUcrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUcrControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZUcrControlWidgetItem* itemDenom(new CZUcrControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZUcrControlWidgetItem *itemMint = new CZUcrControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zUCR seed used to mint this zUCR is not the same as currently hold in the wallet";
            else
...
void ZUcrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUcrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZUcr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUcrControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZUcrControlDialog::GetSelectedMints()
{
...
void ZUcrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
