...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("UltraClear client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("UltraClear"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("UltraClear"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
